...
<script type="text/javascript">
		...
		function openEditDialog(id,mark,zTree,treeNode){
			    ...
			    ok: function () {
					...
					//排列顺序                                                      													
					var orderNum = iframe.document.getElementById('orderNum').value;
					//资源描述
			        ...
			        try{
						...
						var reg = /^[\d]{1,}$/;
						if(!reg.test(orderNum)){throw ("输入正确的排列顺序");}
					}catch (e) {
						$.dialog( {
									time : 3,
									content : e,
					...
					$.ajax({url:"${ctx}/backend/system/resource/edit",type:"post",
						    data: {id:id,parentId:parentId,path:path,name:name,resourceType:resourceType,
						    		methodType : methodType,orderNum:orderNum,description:description,icon:icon
						    	  },dataType:"json",
